...
import selenium_tests.src.po.DeliveryTeacherPage;
...
public class TaskEvaluationTest {
  ...
  public static void setUp() throws MalformedURLException {
    ...
    login.goTo();
    try {
      login.login("Eve", String.valueOf(12345));
   ...
   * (3) Evaluate task
   * (4) Go back to the task list page
   */
  @Test
  public void evaluateFirstTask() {
    tasks.openDelivery(1);
    DeliveryTeacherPage delivery = new DeliveryTeacherPage(driver);
    Assert.assertTrue(delivery.getTaskDetail().isDisplayed());
    Assert.assertTrue(delivery.canBeEvaluated());
    tasks.goToTasks();
